...
namespace Framework
{
    public class FNews
    {
        ...
        public List<Category_News> NEWS_OTHERFIRST(string nid, int top, string lang, string icid)
        {
            ...
            string cmdText = "select top " + top.ToString() + "  " + Sql_News() + " from News where Create_Date > (select Create_Date from News where inid=@nid) and lang=@lang and icid=@icid  and (Create_Date<=getdate() and getdate()<=Modified_Date)  order by Create_Date desc";
            SqlConnection connection = Database.Connection();
            SqlCommand command = new SqlCommand(cmdText, connection)
            {
                CommandType = CommandType.Text
