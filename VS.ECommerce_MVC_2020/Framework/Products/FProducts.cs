...
namespace Framwork
{
    public class Fproducts
    {
        ...
        {
            SqlConnection conn = Database.Connection();
            SqlCommand comm = new SqlCommand("select top " + top.ToString() + " " + Sql_Product() + " from  products where Home=1 and icid in (" + imcid + ") and Status=1 and (Create_Date<=getdate() and getdate()<=Modified_Date) order by Create_Date desc", conn);
            comm.CommandType = CommandType.Text;
