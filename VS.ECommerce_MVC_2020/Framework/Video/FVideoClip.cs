...
namespace Framework
{
    public class FVideoClip
    {
        ...
        {
            string sql = @"select ID,Title,Images,Alt,ImagesSmall,Create_Date,TangName,Youtube,Brief from VideoClip where Menu_ID IN (" + imid + ") and lang=@lang  and Status=@Status and (Create_Date<=getdate() and getdate()<=Modified_Date) order by Create_Date desc";
            SqlConnection conn = Database.Connection();
            SqlCommand comm = new SqlCommand(sql, conn);
            comm.CommandType = CommandType.Text;
