...
namespace VS.ECommerce_MVC.Api
{
    ...
    public class ProductController : ApiControllerBase
    {
        ...
        /// </summary>
        /// <param name="id">id nhóm sản phẩm</param>
        /// <param name="page">Chỉ số phân trang</param>
        ...
        [Route("product/ProductList/{id}/{page}/{pageSize}")]
        [HttpGet]
        public HttpResponseMessage Getcategory(int id, int page = 0, int pageSize = 20)
        {
            int totalRow = 0;
            // var model = SProducts.CategoryDisplay(More.Sub_Menu(More.NS, id.ToString()), "VIE", "1");
            var model = SProducts.Name_Text("SELECT * FROM products WHERE icid in ('" + More.Sub_Menu(More.PR, id.ToString()) + "') and lang='VIE' and Status=1  order by Create_Date desc");
        ...
        /// Tìm kiếm sản phẩm
        /// </summary>
        /// <param name="keyword">Từ khóa sản phẩm</param>
        /// <param name="page">Chỉ số phân trang</param>
        /// <param name="pageSize">Số bản ghi trên mỗi trang</param>
        /// <returns></returns>
        [Route("product/SearchProducts/{keyword}/{page}/{pageSize}")]
        [HttpGet]
        public HttpResponseMessage GetAllKeyword(string keyword, int page = 0, int pageSize = 20)
        {
            int totalRow = 0;
        ...
        /// <summary>
        /// Tìm kiếm sản phẩm lấy theo Count
        /// </summary>
        /// <param name="keyword">Từ khóa sản phẩm</param>
        /// <param name="Count">Số item muốn hiển thị</param>
        /// <returns></returns>
        [Route("product/SearchProductsLoadCount/{keyword}/{Count}")]
        [HttpGet]
        public HttpResponseMessage GetAllKeyword(string keyword, int Count = 10)
        {
        ...
        /// <summary>
        /// Lấy sản phẩm lấy theo Count
        /// </summary>
        /// <param name="keyword">Từ khóa sản phẩm</param>
        /// <param name="Count">Số item muốn hiển thị</param>
        /// <returns></returns>
        [Route("product/ListProductsLoadCount/{Count}")]
        [HttpGet]
        public HttpResponseMessage GetAllLoadCount(int Count = 10)
        {
        ...
        /// <summary>
        /// Lấy sản phẩm theo nhón - lấy theo Count
        /// </summary>
        /// <param name="id">id sản phẩm</param>
        /// <param name="Count">Số item muốn hiển thị</param>
        /// <returns></returns>
        [Route("product/CategoryProductsLoadCount/{id}/{Count}")]
        [HttpGet]
        public HttpResponseMessage CategoryLoadCount(int id, int Count = 10)
        {
                var query = SProducts.Name_Text("select  top " + Count.ToString() + " * from products where icid in ('" + More.Sub_Menu(More.PR, id.ToString()) + "') and  lang='VIE'  order by Create_Date desc");
                var responseData = Mapper.Map<IEnumerable<Entity.Products>, IEnumerable<ProductViewModel>>(query);
                var response = Request.CreateResponse(HttpStatusCode.OK, responseData);
                return response;
        }
        ...
        /// <summary>
        ///  Export Exel danh sách sản phẩm
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        ...
        /// <summary>
        ///  Export Pdf danh sách sản phẩm
        /// </summary>
        /// <returns></returns>
        [HttpGet]
        [Route("product/ExportPdf")]
        public async Task<HttpResponseMessage> ExportPdf(int id)
        {
