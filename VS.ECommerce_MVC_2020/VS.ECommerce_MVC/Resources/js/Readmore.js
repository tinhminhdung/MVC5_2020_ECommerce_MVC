(function (root) {
	...
	function setBoxHeights(element) {
		...
		clonedElement.style.width = element.getBoundingClientRect()
			.width;
		clonedElement.style.overflow = 'hidden';
		element.parentNode.insertBefore(clonedElement, element);
		clonedElement.style.maxHeight = 'none';
		expandedHeight = parseInt(clonedElement.getBoundingClientRect()
			.height, 10);
		cssMaxHeight = parseInt(getComputedStyle(clonedElement)
			.maxHeight, 10);
	...
	Readmore = (function () {
		...
		defaults = {
			...
			, collapsedHeight:256
			, heightMargin: 16
			, moreLink: '<a href="#" class="view-less">Xem thêm</a>'
			, lessLink: '<a href="#"  class="view-less">Rút gọn</a>'
			, embedCSS: true
		...
		function Readmore(selector, options) {
			...
			forEach(document.querySelectorAll(selector), function (i, element) {
				...
				if (element.getBoundingClientRect()
					.height <= element.readmore.collapsedHeight + heightMargin) {
		...
		Readmore.prototype.toggle = function (trigger, element, event) {
			...
			expanded = element.getBoundingClientRect()
				.height <= element.readmore.collapsedHeight;
